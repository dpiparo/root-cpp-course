...
int main()
{
    // Opens the file and immediately wraps it around a std::unique_ptr. The memory will be automatically managed by it.
    // TFile::Open allows to seamlessly read remote files
    std::unique_ptr<TFile> f{TFile::Open("root://eospublic.cern.ch//eos/root-eos/hsimple.root")};
    ...
    // The type of the histogram is TH1F in this case, meaning the bins are of type `float`
